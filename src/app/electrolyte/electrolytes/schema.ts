...
export const naCorrectionSchema = z.object({
  na: z.preprocess(
    prep,
    z.coerce.number({ invalid_type_error: "血清Naを入力してください" })
      .min(100, "血清Naは100mEq/L以上で入力してください")
      .max(180, "血清Naは180mEq/L以下で入力してください")
  ),
  glucose: z.preprocess(
    prep,
    z.coerce.number({ invalid_type_error: "血糖値を入力してください" })
...
export const kCorrectionSchema = z.object({
  k: z.preprocess(
    prep,
    z.coerce.number({ invalid_type_error: "血清Kを入力してください" })
      .min(1, "血清Kは1mEq/L以上で入力してください")
      .max(10, "血清Kは10mEq/L以下で入力してください")
  ),
  ph: z.preprocess(
    prep,
    z.coerce.number({ invalid_type_error: "pHを入力してください" })
